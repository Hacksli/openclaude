...
const externalTips: Tip[] = [
  {
    id: 'new-user-warmup',
    content: async () =>
      `Start with small features or bug fixes, tell the agent to propose a plan, and verify its suggested edits`,
    cooldownSessions: 3,
    async isRelevant() {
  ...
  },
  {
    ...
    content: async () =>
      `Режим планування — для підготовки до складного запиту. Натисни ${getShortcutDisplay('chat:cycleMode', 'Chat', 'shift+tab')} двічі.`,
    cooldownSessions: 5,
    isRelevant: async () => {
      const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async () =>
      `Використай /config щоб змінити режим дозволів за замовчуванням (включно з режимом планування)`,
    cooldownSessions: 10,
    isRelevant: async () => {
      try {
        const config = getGlobalConfig()
  ...
  {
    id: 'memory-command',
    content: async () => 'Use /memory to view and manage memory',
    cooldownSessions: 15,
    async isRelevant() {
      const config = getGlobalConfig()
  ...
  },
  {
    id: 'todo-list',
    content: async () =>
      'Ask the agent to create a todo list when working on complex tasks to track progress and remain on track',
    cooldownSessions: 20,
    isRelevant: async () => true,
  },
  {
  ...
  },
  {
    ...
    content: async () => 'Connect to your IDE · /ide',
    cooldownSessions: 4,
    async isRelevant() {
  ...
  },
  {
    ...
    content: async () => 'Run /install-slack-app to use in Slack',
    cooldownSessions: 10,
  ...
  },
  {
    id: 'paste-images-mac',
    content: async () =>
      'Paste images into Neural Network Coder using control+v (not cmd+v!)',
    cooldownSessions: 10,
  ...
  },
  {
    id: 'continue',
    content: async () =>
      'Run nnc --continue or nnc --resume to resume a conversation',
    cooldownSessions: 10,
    isRelevant: async () => true,
  },
  {
  ...
  },
  {
    id: 'custom-commands',
    content: async () =>
      'Create skills by adding .md files to .nnc/skills/ in your project or ~/.nnc/skills/ for skills that work in any project',
    cooldownSessions: 15,
    async isRelevant() {
      const config = getGlobalConfig()
      return config.numStartups > 10
  ...
  },
  {
    id: 'desktop-app',
    content: async () =>
      'Run Neural Network Coder locally or remotely using the desktop app: clau.de/desktop',
    cooldownSessions: 15,
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return `Continue your session in Neural Network Coder Desktop with ${blue('/desktop')}`
    },
    cooldownSessions: 15,
    isRelevant: async () => {
  ...
  },
  {
    id: 'mobile-app',
    content: async () =>
      '/mobile to use Neural Network Coder from the mobile app on your phone',
    cooldownSessions: 15,
    isRelevant: async () => true,
  },
  {
    ...
    content: async () =>
      `Стандартна модель — Opus Plan Mode. Натисни ${getShortcutDisplay('chat:cycleMode', 'Chat', 'shift+tab')} двічі щоб активувати режим планування.`,
    cooldownSessions: 2,
    async isRelevant() {
      const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return variant === 'copy_b'
        ? `For big tasks, tell the agent to ${blue('use subagents')}. They work in parallel and keep your main thread clean.`
        : `Say ${blue('"fan out subagents"')} and the agent sends a team. Each one digs deep so nothing gets missed.`
    },
    cooldownSessions: 3,
    isRelevant: async () => {
      if (!is1PApiCustomer()) return false
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return reward
        ? `Share Neural Network Coder and earn ${claude(formatCreditAmount(reward))} of extra usage · ${claude('/passes')}`
