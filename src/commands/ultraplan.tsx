    ...
    try {
      ...
      if (executionTarget === 'remote') {
        ...
        enqueuePendingNotification({
          value: [`Ultraplan approved — executing in Neural Network Coder on the web. Follow along at: ${url}`, '', 'Results will land as a pull request when the remote session finishes. There is nothing to do here.'].join('\n'),
...
function buildLaunchMessage(disconnectedBridge?: boolean): string {
  ...
  return `${DIAMOND_OPEN} ultraplan\n${prefix}Starting Neural Network Coder on the web…`;
}
function buildSessionReadyMessage(url: string): string {
  return `${DIAMOND_OPEN} ultraplan · Monitor progress in Neural Network Coder on the web ${url}\nYou can continue working — when the ${DIAMOND_OPEN} fills, press ↓ to view results`;
}
...
export async function launchUltraplan(opts: {
  ...
  if (!blurb && !seedPlan) {
    ...
    'Usage: /ultraplan \\<prompt\\>, or include "ultraplan" anywhere', 'in your prompt', '', 'Advanced multi-agent plan mode with our most powerful model', '(Opus). Runs in Neural Network Coder on the web. When the plan is ready,', 'you can execute it in the web session or send it back here.', 'Terminal stays free while the remote plans.', 'Requires /login.', '', `Terms: ${CCR_TERMS_URL}`].join('\n');
  }
...
export default {
  type: 'local-jsx',
  name: 'ultraplan',
  description: `~10–30 min · Neural Network Coder on the web drafts an advanced plan you can edit and approve. See ${CCR_TERMS_URL}`,
  argumentHint: '<prompt>',
