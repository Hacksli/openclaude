...
export const AUTO_MODE_DESCRIPTION = "Auto mode handles permission prompts automatically — each tool call is checked for risky actions and prompt injection before executing. Actions identified as safe are executed, while actions identified as risky are blocked and a different approach may be tried. Ideal for long-running tasks. Sessions are slightly more expensive. Mistakes can allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
