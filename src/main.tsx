...
import { cacheSessionTitle, getSessionIdFromLog, loadTranscriptFromFile, saveAgentSetting, saveMode, saveModel, saveStrategyMode, searchSessionsByCustomTitle, sessionIdExists } from './utils/sessionStorage.js';
...
/* eslint-disable @typescript-eslint/no-require-imports */
const autoModeStateModule = false ? require('./utils/permissions/autoModeState.js') as typeof import('./utils/permissions/autoModeState.js') : null;
...
// Set by early argv processing when `nnc open <url>` is detected (interactive mode only)
type PendingConnect = {
...
// Set by early argv processing when `nnc assistant [sessionId]` is detected
...
// `nnc ssh <host> [dir]` — parsed from argv early (same pattern as
...
export async function main() {
  ...
  // `nnc assistant [sessionId]` — stash and strip so the main
  ...
  // `nnc -p "explain assistant"`. Root-flag-before-subcommand
  ...
  if (false && _pendingAssistantChat) {
    ...
    if (rawArgs[0] === 'assistant') {
      ...
      // else: `nnc assistant --help` → fall through to stub
    }
  }

  // `nnc ssh <host> [dir]` — strip from argv so the main command handler
  ...
  if (false && _pendingSSH) {
    ...
    // given, so `nnc ssh --permission-mode auto host` and `nnc ssh host
    ...
    if (rawCliArgs[0] === 'ssh') {
      ...
      // (which persists under the remote's ~/.nnc/projects/<cwd>/).
  ...
  const clientType = (() => {
    ...
    if (process.env.CLAUDE_CODE_ENTRYPOINT === 'nnc-vscode') return 'nnc-vscode';
  ...
  // Tag sessions created via `nnc remote-control` so the backend can identify them
...
async function run(): Promise<CommanderCommand> {
  ...
  program.name('claude').description(`Neural Network Coder - starts an interactive session by default, use -p/--print for non-interactive output`).argument('[prompt]', 'Your prompt', String)
  ...
  .helpOption('-h, --help', 'Display help for command').option('-d, --debug [filter]', 'Enable debug mode with optional category filtering (e.g., "api,hooks" or "!1p,!file")', (_value: string | true) => {
    ...
    return true;
  }).addOption(new Option('-d2e, --debug-to-stderr', 'Enable debug mode (to stderr)').argParser(Boolean).hideHelp()).option('--debug-file <path>', 'Write debug logs to a specific file path (implicitly enables debug mode)', () => true).option('--verbose', 'Override verbose mode setting from config', () => true).option('-p, --print', 'Print response and exit (useful for pipes). Note: The workspace trust dialog is skipped when the CLI is run with the -p mode. Only use this flag in directories you trust.', () => true).option('--bare', 'Minimal mode: skip hooks, LSP, plugin sync, attribution, auto-memory, background prefetches, keychain reads, and NNC.md auto-discovery. Sets CLAUDE_CODE_SIMPLE=1. Anthropic auth is strictly ANTHROPIC_API_KEY or apiKeyHelper via --settings (OAuth and keychain are never read). 3P providers (Bedrock/Vertex/Foundry) use their own credentials. Skills still resolve via /skill-name. Explicitly provide context via: --system-prompt[-file], --append-system-prompt[-file], --add-dir (NNC.md dirs), --mcp-config, --settings, --agents, --plugin-dir.', () => true).option('--strategymode', 'Enable strategy mode: four-stage workflow (analysis → proposal → plan → execution) with confirmation gates at each step. Read-only during stages 1\u20133.', () => true).option('--cloudeconf', 'Also read sessions from the legacy ~/.claude config tree (read-only merge) so /resume surfaces pre-migration transcripts.', () => true).addOption(new Option('--init', 'Run Setup hooks with init trigger, then continue').hideHelp()).addOption(new Option('--init-only', 'Run Setup and SessionStart:startup hooks, then exit').hideHelp()).addOption(new Option('--maintenance', 'Run Setup hooks with maintenance trigger, then continue').hideHelp()).addOption(new Option('--output-format <format>', 'Output format (only works with --print): "text" (default), "json" (single result), or "stream-json" (realtime streaming)').choices(['text', 'json', 'stream-json'])).addOption(new Option('--json-schema <schema>', 'JSON Schema for structured output validation. ' + 'Example: {"type":"object","properties":{"name":{"type":"string"}},"required":["name"]}').argParser(String)).option('--include-hook-events', 'Include all hook lifecycle events in the output stream (only works with --output-format=stream-json)', () => true).option('--include-partial-messages', 'Include partial message chunks as they arrive (only works with --print and --output-format=stream-json)', () => true).addOption(new Option('--input-format <format>', 'Input format (only works with --print): "text" (default), or "stream-json" (realtime streaming input)').choices(['text', 'stream-json'])).option('--mcp-debug', '[DEPRECATED. Use --debug instead] Enable MCP debug mode (shows MCP server errors)', () => true).option('--dangerously-skip-permissions', 'Bypass all permission checks. Recommended only for sandboxes with no internet access.', () => true).option('--allow-dangerously-skip-permissions', 'Enable bypassing all permission checks as an option, without it being enabled by default. Recommended only for sandboxes with no internet access.', () => true).addOption(new Option('--thinking <mode>', 'Thinking mode: enabled (equivalent to adaptive), disabled').choices(['enabled', 'adaptive', 'disabled']).hideHelp()).addOption(new Option('--max-thinking-tokens <tokens>', '[DEPRECATED. Use --thinking instead for newer models] Maximum number of thinking tokens (only works with --print)').argParser(Number).hideHelp()).addOption(new Option('--max-turns <turns>', 'Maximum number of agentic turns in non-interactive mode. This will early exit the conversation after the specified number of turns. (only works with --print)').argParser(Number).hideHelp()).addOption(new Option('--max-budget-usd <amount>', 'Maximum dollar amount to spend on API calls (only works with --print)').argParser(value => {
  ...
  // --flag. `nnc --plugin-dir /path mcp add --transport http` swallowed
  // `mcp` and `add` as paths, then choked on --transport as an unknown
  // top-level option. Single-value + collect accumulator means each
  // --plugin-dir takes exactly one arg; repeat the flag for multiple dirs.
  .option('--plugin-dir <path>', 'Load plugins from a directory for this session only (repeatable: --plugin-dir A --plugin-dir B)', (val: string, prev: string[]) => [...prev, val], [] as string[]).option('--disable-slash-commands', 'Disable all skills', () => true).option('--chrome', 'Enable Chrome Extension integration').option('--no-chrome', 'Disable Chrome Extension integration').option('--file <specs...>', 'File resources to download at startup. Format: file_id:relative_path (e.g., --file file_abc:doc.txt file_def:img.png)').action(async (prompt, options) => {
    profileCheckpoint('action_handler_start');

    // --bare = one-switch minimal mode. Sets SIMPLE so all the existing
    // gates fire (NNC.md, skills, hooks inside executeHooks, agent
    ...
    if (prompt === 'code') {
      ...
      console.warn(chalk.yellow('Tip: You can launch Neural Network Coder with just `nnc`'));
      prompt = undefined;
    }
    ...
    // Assistant mode: when .nnc/settings.json has assistant: true AND
    ...
    //
    // Trust gate: .nnc/settings.json is attacker-controllable in an
    // untrusted clone. We run ~1000 lines before showSetupScreens() shows
    // the trust dialog, and by then we've already appended
    // .nnc/agents/assistant.md to the system prompt. Refuse to activate
    ...
    // Extract Neural Network Coder in Chrome option and enforce claude.ai subscriber check (unless user is ant)
    ...
    if (enableClaudeInChrome) {
      ...
      try {
        ...
        logForDebugging(`[Chrome Extension] Error: ${error}`);
        logError(error);
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(`Error: Failed to run with Chrome Extension.`);
        process.exit(1);
      }
      ...
      try {
        ...
        logForDebugging(`[Chrome Extension] Error (auto-enable): ${error}`);
      }
    }
    ...
    // Store additional directories for NNC.md loading (controlled by env var)
    ...
    if (false && dangerousPermissions.length > 0) {
    ...
    if (getIsNonInteractiveSession()) {
      ...
      // .nnc/settings.json PATH/GIT_DIR/GIT_WORK_TREE) so gitExe() and
      ...
      // getMemoryFiles) yields naturally, so the NNC.md directory walk
    ...
    // Cache model and --strategymode state so /resume shows them in the
    // picker. saveModel/saveStrategyMode only cache in-memory; the values
    // land on disk when materializeSessionFile runs on the first user
    // message (via reAppendSessionMetadata).
    saveModel(resolvedInitialModel);
    // Only record strategy mode when it's on. On --resume, we want the
    // picker's "this was a strategy session" tag to persist across resumes
    // where the user didn't re-pass --strategymode; recording `false` would
    // clobber the original session's tag on the next reAppend.
    if (process.env.NNC_STRATEGY_MODE === '1') {
      saveStrategyMode(true);
    }
    ...
    // Register PID file for concurrent-session detection (~/.nnc/sessions/)
    // and fire multi-clauding telemetry. Lives here (not init.ts) so only the
    // REPL path registers — not subcommands like `nnc doctor`. Chained:
    ...
    if (options.continue) {
      ...
      // `nnc connect <url>` — full interactive TUI connected to a remote server
      ...
      // `nnc ssh <host> [dir]` — probe remote, deploy binary if needed,
      ...
      // `nnc assistant [sessionId]` — REPL as a pure viewer client
      ...
      if (!targetSessionId) {
        ...
        if (sessions.length === 0) {
          ...
          return await exitWithMessage(root, `Assistant installed in ${installedDir}. The daemon is starting up — run .nnc assistant\` again in a few seconds to connect.`, {
            exitCode: 0,
      ...
      // --remote and --teleport both create/resume Neural Network Coder Web (CCR) sessions.
      ...
      if (remote !== null) {
        ...
        if (!isRemoteTuiEnabled) {
          ...
          process.stdout.write(`Resume with: nnc --teleport ${createdSession.id}\n`);
          await gracefulShutdown(0);
          process.exit(0);
        }
            ...
            if (repoValidation.status === 'mismatch' || repoValidation.status === 'not_in_repo') {
              ...
              if (sessionRepo) {
                ...
                if (existingPaths.length > 0) {
                  ...
                  throw new TeleportOperationError(`You must run nnc --teleport ${teleport} from a checkout of ${sessionRepo}.`, chalk.red(`You must run nnc --teleport ${teleport} from a checkout of ${chalk.bold(sessionRepo)}.\n`));
                }
              }
      ...
      // prompt — and the working directory / NNC.md it implies — came
  ...
  }).version(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Neural Network Coder)`, '-v, --version', 'Output the version number');
  ...
  if ("external" === 'ant') {
    ...
    program.option('--agent-teams', '[internal-only] Force multi-agent mode for solving problems', () => true);
  }
  if (false) {
  ...
  mcp.command('serve').description(`Start the Neural Network Coder MCP server`).option('-d, --debug', 'Enable debug mode', () => true).option('--verbose', 'Override verbose mode setting from config', () => true).action(async ({
    debug,
    verbose
  }: {
    debug?: boolean;
  ...
  // `nnc ssh <host> [dir]` — registered here only so --help shows it.
  ...
  if (false) {
    program.command('open <cc-url>').description('Connect to a Neural Network Coder server (internal — use cc:// URLs)').option('-p, --print [prompt]', 'Print mode (headless)').option('--output-format <format>', 'Output format: text, json, stream-json', 'text').action(async (ccUrl: string, opts: {
  ...
  auth.command('login').description('Sign in to your Anthropic account').option('--email <email>', 'Pre-populate email address on the login page').option('--sso', 'Force SSO login flow').option('--console', 'Use Anthropic Console (API usage billing) instead of subscription').option('--claudeai', 'Use subscription (default)').action(async ({
    email,
    sso,
    console: useConsole,
    claudeai
  ...
  const pluginCmd = program.command('plugin').alias('plugins').description('Manage Neural Network Coder plugins').configureHelp(createSortedHelpConfig());
  ...
  const marketplaceCmd = pluginCmd.command('marketplace').description('Manage Neural Network Coder marketplaces').configureHelp(createSortedHelpConfig());
  marketplaceCmd.command('add <source>').description('Add a marketplace from a URL, path, or GitHub repo').addOption(coworkOption()).option('--sparse <paths...>', 'Limit checkout to specific directories via git sparse-checkout (for monorepos). Example: --sparse .nnc-plugin plugins').option('--scope <scope>', 'Where to declare the marketplace: user (default), project, or local').action(async (source: string, options: {
    cowork?: boolean;
    sparse?: string[];
    scope?: string;
  }) => {
  ...
  pluginCmd.command('uninstall <plugin>').alias('remove').alias('rm').description('Uninstall an installed plugin').option('-s, --scope <scope>', 'Uninstall from scope: user, project, or local', 'user').option('--keep-data', "Preserve the plugin's persistent data directory (~/.nnc/plugins/data/{id}/)").addOption(coworkOption()).action(async (plugin: string, options: {
    scope?: string;
    cowork?: boolean;
    keepData?: boolean;
  }) => {
  ...
  // Setup token command
  program.command('setup-token').description('Set up a long-lived authentication token (requires subscription)').action(async () => {
    const [{
      setupTokenHandler
    }, {
      createRoot
  ...
  program.command('doctor').description('Check the health of your Neural Network Coder auto-updater. Note: The workspace trust dialog is skipped and stdio servers from .mcp.json are spawned for health checks. Only use this command in directories you trust.').action(async () => {
    const [{
      doctorHandler
    }, {
      createRoot
  ...
  // claude up — run the project's NNC.md "# claude up" setup instructions.
  if ("external" === 'ant') {
    program.command('up').description('[internal-only] Initialize or upgrade the local dev environment using the "# claude up" section of the nearest NNC.md').action(async () => {
      const {
        up
  ...
  // claude install
  program.command('install [target]').description('Install Neural Network Coder native build. Use [target] to specify version (stable, latest, or specific version)').option('--force', 'Force installation even if already installed').action(async (target: string | undefined, options: {
    force?: boolean;
  }) => {
    const {
      installHandler
