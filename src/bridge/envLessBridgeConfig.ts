...
export async function checkEnvLessBridgeMinVersion(): Promise<string | null> {
  ...
  if (cfg.min_version && lt(MACRO.VERSION, cfg.min_version)) {
    return `Your version of Neural Network Coder (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${cfg.min_version} or higher is required. Run .nnc update\` to update.`
  }
  return null
}
