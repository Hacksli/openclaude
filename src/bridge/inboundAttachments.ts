 ...
 * (oauth-authed, same store), write to ~/.nnc/uploads/{sessionId}/, and
 * return @path refs to prepend. Neural Network Coder's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches Neural Network Coder, just without @path.
 */
