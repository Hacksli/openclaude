 ...
 * =====================
 * This module manages an isolated tmux socket for Neural Network Coder's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, Neural Network Coder could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started Neural Network Coder from within tmux.
 *
 * HOW IT WORKS:
 * 1. Neural Network Coder creates its own tmux socket: `claude-<PID>` (e.g., `claude-12345`)
 ...
 *
 * This means ANY tmux command run through Neural Network Coder - whether via the Tmux tool
 * directly or via Bash - will operate on Neural Network Coder's isolated socket, NOT the
 * user's tmux session.
 *
...
/**
 * Gets the socket name for Neural Network Coder's isolated tmux session.
 * Format: claude-<PID>
 */
...
/**
 * Gets the TMUX environment variable value for Neural Network Coder's isolated socket.
 *
 ...
 * the Bash tool will operate on Neural Network Coder's socket, NOT the user's session.
 *
...
/**
 * Kills the tmux server for Neural Network Coder's isolated socket.
...
async function doInitialize(): Promise<void> {
  ...
  // Any Neural Network Coder instance spawned on this socket will inherit this env var,
