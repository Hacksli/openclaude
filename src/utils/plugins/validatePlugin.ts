 ...
 * one into the other. Surfaced as warnings by `nnc plugin validate` since
...
function detectManifestType(
  ...
  // Check if it's in .nnc-plugin directory
  if (dirName === '.nnc-plugin') {
 ...
 * '..' a user added to "climb out of .nnc-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .nnc-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .nnc-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .nnc-plugin/), not relative to marketplace.json. ' +
...
export async function validatePluginManifest(
  ...
  // them. `nnc plugin validate` is a developer tool — authors running it
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.nnc-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.nnc-plugin/plugin.json says "${manifestVersion}". ` +
 ...
 * for the load path, but authors running `nnc plugin validate` want a hard
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and the agent ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .nnc-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.nnc-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .nnc-plugin/marketplace.json or .nnc-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
