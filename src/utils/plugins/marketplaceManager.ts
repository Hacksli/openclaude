/**
 * Marketplace manager for Neural Network Coder plugins
 *
 ...
 *
 * Reads the configuration file at ~/.nnc/plugins/known_marketplaces.json
 ...
 *     "installLocation": "/Users/me/.nnc/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.nnc/plugins/marketplaces/company-plugins",
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Neural Network Coder bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.nnc/plugins/known_marketplaces.json,
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'Neural Network Coder-Code-Plugin-Manager',
  }
 ...
 * - GitHub: Clones repo and looks for .nnc-plugin/marketplace.json
    ...
    switch (source.source) {
      ...
      case 'github': {
        ...
        marketplacePath = join(
          temporaryCachePath,
          source.path || '.nnc-plugin/marketplace.json',
        )
        break
      }
      ...
      case 'file': {
        ...
        // File sources point to .nnc-plugin/marketplace.json, so the marketplace
        // root is two directories up (parent of .nnc-plugin/)
      ...
      case 'directory': {
        // For directories, look for .nnc-plugin/marketplace.json
        // Resolve to absolute so error messages show the actual path checked
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.nnc-plugin', 'marketplace.json')
 ...
 * The configuration is saved to ~/.nnc/plugins/known_marketplaces.json.
 *
  ...
  if (seedDir) {
    throw new Error(
      `Marketplace '${name}' is registered from the read-only seed directory ` +
        ...
        `To stop using its plugins: nnc plugin disable <plugin>@${name}`,
    )
  }
...
async function readCachedMarketplace(
  ...
  // For git-sourced directories, the manifest lives at .nnc-plugin/marketplace.json.
  ...
  const nestedPath = join(installLocation, '.nnc-plugin', 'marketplace.json')
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Neural Network Coder version. Run 'claude marketplace remove ${name}' and ` +
...
export async function refreshAllMarketplaces(): Promise<void> {
  ...
  for (const [name, entry] of Object.entries(config)) {
    ...
    // hits this path on `nnc plugin marketplace update` (no name arg).
      ...
      if (resolvedLoc !== cacheDir && !resolvedLoc.startsWith(cacheDir + sep)) {
        throw new Error(
          `Marketplace '${name}' has a corrupted installLocation ` +
            ...
            `Run: nnc plugin marketplace remove "${name}" and re-add it.`,
        )
      }
    }
      ...
      try {
        ...
        throw new Error(
          `The marketplace.json file is no longer present in this repository.\n\n` +
            ...
            `You can remove this marketplace with: nnc plugin marketplace remove "${name}"`,
        )
      }
