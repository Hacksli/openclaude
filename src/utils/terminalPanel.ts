 ...
 * socket (e.g., "claude-panel-a1b2c3d4") holds the shell session. Each Neural Network Coder
 ...
 * Neural Network Coder while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per Neural Network Coder instance (based on session ID)
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to Neural Network Coder from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to the REPL ', ';',
