...
/**
 * If filePath is inside a .nnc/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .nnc/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.nnc', 'skills')),
      prefix: '/.nnc/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.nnc', 'skills')),
      prefix: '~/.nnc/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.nnc/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.nnc/./settings.json` which would evade the endsWith() check
  ...
  if (
    ...
    // Include .nnc/settings.json even for other projects
    return true
  }
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  // Check if file is within .nnc/commands or .nnc/agents directories
 ...
 * Path format: ~/.nnc/projects/{sanitized-cwd}/...
 */
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `Requested write access to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isClaudeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `Requested write access to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `Requested edit access to ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .nnc/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `Requested access to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `Requested read access to ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `Requested read access to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `Requested read access to ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // 1.6. Check for .nnc/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .nnc/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .nnc/ folder.
  //
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .nnc/ (project or global).
    // Accepts both the broad patterns ('/.nnc/**', '~/.nnc/**') and
    // narrowed ones like '/.nnc/skills/my-skill/**' so users can grant
    ...
    // '/.nnc/../**' from leaking this bypass outside .nnc/.
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .nnc/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.nnc/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.nnc/jobs/. Symlink guard: every
  ...
  if (false) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.nnc is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.nnc/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .nnc/ only (not ~/.nnc/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.nnc', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.nnc/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.nnc/teams/) for swarm coordination
