...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.nnc/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.nnc/settings.json)'
    case 'projectSettings':
      return 'Project settings (.nnc/settings.json)'
    case 'localSettings':
      return 'Local settings (.nnc/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.nnc/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.nnc/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
