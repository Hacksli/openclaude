...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error('Neural Network Coder web sessions require authentication. API key authentication is not sufficient. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
...
/**
 * Creates a remote Neural Network Coder.ai session with error handling and UI feedback.
...
/**
 * Creates a remote Neural Network Coder.ai session using the Sessions API.
 *
 * Two source modes:
