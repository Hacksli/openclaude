...
test('install command displays ~/.local/bin/nnc on non-Windows', async () => {
  ...
  expect(getInstallationPath()).toBe('~/.local/bin/nnc')
})

test('install command displays nnc.exe path on Windows', async () => {
  mock.module('../utils/env.js', () => ({
    env: { platform: 'win32' },
  }))
  ...
  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'nnc.exe').replace(/\//g, '\\'),
  )
})

test('cleanupNpmInstallations removes both nnc and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    getClaudeConfigHomeDir: () => join(homedir(), '.nnc'),
