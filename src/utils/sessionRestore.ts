 ...
 * Used for computing initial state before render (per NNC.md guidelines).
...
export async function processResumedConversation(
  ...
  if (!opts.forkSession) {
    ...
    // the pre-loaded records). Without this seed, `nnc -r {newSessionId}`
  ...
  // Compute initial state before render (per NNC.md guidelines)
