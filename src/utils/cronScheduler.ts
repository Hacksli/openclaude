// Non-React scheduler core for .nnc/scheduled_tasks.json.
...
type CronSchedulerOptions = {
  ...
  /**
   * Directory containing .nnc/scheduled_tasks.json. When provided, the
...
export function createCronScheduler(
  ...
  async function load(initial: boolean) {
    ...
    // and fires immediately). This avoids a misleading "missed while Neural Network Coder
  ...
  function check() {
    ...
    // exists to stop two Neural Network Coder sessions in the same cwd from double-firing
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    ...
    `${plural ? 'They have' : 'It has'} already been removed from .nnc/scheduled_tasks.json.\n\n` +
