...
const missingCodexAuthPath = join(tmpdir(), 'nnc-missing-codex-auth.json')
...
test('codex launch prefers auth account id over stale persisted value', async () => {
  const codexHome = mkdtempSync(join(tmpdir(), 'nnc-codex-'))
  try {
    writeFileSync(
...
test('saveProfileFile writes a profile that loadProfileFile can read back', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'nnc-profile-file-'))
...
test('clearPersistedCodexOAuthProfile removes only persisted Codex OAuth profiles', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'nnc-codex-oauth-profile-'))
