...
export function splitSysPromptPrefix(
  ...
  if (useGlobalCacheFeature && options?.skipGlobalCacheForSystemPrompt) {
    ...
    for (const prompt of systemPrompt) {
      ...
      if (prompt.startsWith('x-client-trace-header')) {
  ...
  if (useGlobalCacheFeature) {
    ...
    if (boundaryIndex !== -1) {
      ...
      for (let i = 0; i < systemPrompt.length; i++) {
        ...
        if (block.startsWith('x-client-trace-header')) {
...
export function normalizeToolInput<T extends Tool>(
  ...
  switch (tool.name) {
    ...
    case BashTool.name: {
      ...
      // Logging for commands that are only echoing a string. This is to help us understand how often  Neural Network Coder talks via bash
