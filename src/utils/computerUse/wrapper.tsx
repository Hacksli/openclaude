 ...
 * rule (src/NNC.md): the dispatcher closure must persist across calls so
...
function formatLockHeld(holder: string): string {
  return `Computer use is in use by another session (${holder.slice(0, 8)}…). Wait for that session to finish or run /exit there.`;
}
