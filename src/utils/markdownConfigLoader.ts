...
// Neural Network Coder configuration directory names
 ...
 * stopping there makes the parent project's `.nnc/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.nnc/worktrees/`) stay on the old behavior: their `.git`
 ...
 * from leaking into projects. For example, if ~/projects/.nnc/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .nnc/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.nnc', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.nnc', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .nnc/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .nnc/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .nnc/<subdir> content — loading the main
    ...
    // This prevents the same file from appearing multiple times when ~/.nnc is
 ...
 * @param dir Directory (eg. "~/.nnc/commands")
 * @returns Array of parsed markdown files with metadata
 */
