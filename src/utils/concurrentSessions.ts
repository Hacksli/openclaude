...
/**
 * Kind override from env. Set by the spawner (`nnc --bg`, daemon
...
/**
 * True when this REPL is running inside a `nnc --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `nnc ps` sees everything
...
export async function registerSession(): Promise<boolean> {
  ...
  try {
    ...
    // this, the PID file's sessionId goes stale and `nnc ps` sparkline
...
/**
 * Push live activity state for `nnc ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.nnc/sessions/ is shared with Windows-native Neural Network Coder (symlink
