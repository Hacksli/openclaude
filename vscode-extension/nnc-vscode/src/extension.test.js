...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'nnc',
    launchCommand: 'nnc --project-aware',
    terminalName: 'Neural Network Coder',
    shimEnabled: false,
    workspaceFolder: '/workspace/nnc/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/nnc/very/long/path/example-project',
    launchCwdLabel: '/workspace/nnc/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/nnc/very/long/path/example-project/.nnc-profile.json',
    workspaceProfilePath: '/workspace/nnc/very/long/path/example-project/.nnc-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Neural Network Coder wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/nnc\/very\/long\/path\/example-project"[^>]*>\/workspace\/nnc\/very\/long\/path\/example-project<\//,
  );
});
...
test('NeuralNetworkCoderControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { NeuralNetworkCoderControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/nnc/src/panels/control-center.js',
      workspacePath: '/workspace/nnc',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/nnc/src/panels',
      projectAwareCwdLabel: '/workspace/nnc/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/nnc',
      workspaceRootCwdLabel: '/workspace/nnc',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/nnc',
      activeFilePath: '/workspace/nnc/src/panels/control-center.js',
      workspacePath: '/workspace/nnc',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/nnc',
      projectAwareCwdLabel: '/workspace/nnc',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/nnc/src/panels',
      launchCwdLabel: '/workspace/nnc/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/nnc\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/nnc/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/nnc',
      launchCwdLabel: '/workspace/nnc',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/nnc/);
  assert.match(html, /Same workspace-root target as Launch Neural Network Coder because the relative command resolves from the workspace root · \/workspace\/nnc/);
});
